...
int main(int argc, char* argv[]) {
    ...
    //make client
    ClientManager clientManager("localhost", 20020, &ringBuffer);
    //set the ring buffer for the window manager
    windowManager.setRingBuffer(&ringBuffer);
    //make thread for incoming messages
    thread incomingMessageListener(&ClientManager::incomingListener, ref(clientManager),
                                   bind(&WindowManager::updateMessageWindow, ref(windowManager)));
    //start getting user input
    windowManager.getUserInput(bind(&ClientManager::sendMessage, ref(clientManager), placeholders::_1));
    //kill the listener
